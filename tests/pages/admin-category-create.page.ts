...
export type CategoryCreateOptions = {
  /** 대분류명 (한국어) */
  nameKr: string;
  /** 대분류명 (영어) */
  nameEn: string;
...
export class CategoryCreatePage extends AdminBasePage {
  ...
  constructor(page: Page) {
    ...
    this.distributorCombobox = page
      .locator('div:has(> div:has-text("유통사를 선택해주세요"))')
      .first();
    this.artistCombobox = page
      .locator('div:has(> div:has-text("아티스트를 선택해주세요"))')
      .first();

    // 발매일 입력
    this.releaseDateInput = page.getByPlaceholder(
      "발매일을 입력해주세요 (YYYY-MM-DD)",
    );

    // 이미지 업로드 영역
    this.imageUploadArea = page
      .locator("text=여기로 파일을 드래그")
      .locator("..");
  ...
  async fillNameKr(name: string): Promise<void> {
    await this.nameKrInput.waitFor({
      state: "visible",
  ...
  async fillNameEn(name: string): Promise<void> {
    await this.nameEnInput.waitFor({
      state: "visible",
      timeout: this.timeouts.medium,
    });
  ...
  async selectDistributor(searchTerm: string): Promise<void> {
    ...
    const distributorContainer = this.page
      .locator('.multiselect:has([placeholder*="유통사"])')
      .first();

    // fallback: 첫 번째 multiselect
    const multiselectContainer = (await distributorContainer
      .isVisible({ timeout: 1000 })
      .catch(() => false))
    ...
    const searchInput = this.page
      .locator("input.multiselect__input:visible")
      .first();

    // input이 보이면 검색어 입력
    const isInputVisible = await searchInput
    ...
    if (await matchingOption.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (
        await firstValidOption.isVisible({ timeout: 1000 }).catch(() => false)
      ) {
        ...
        if (
          ...
          console.warn(
            "⚠️ 유효한 유통사 옵션을 찾을 수 없음 - 검색 결과가 비어있음",
          );
        }
      } else {
        console.warn("⚠️ 유통사 옵션을 찾을 수 없음");
      }
    }
  ...
  async selectArtist(artistName: string): Promise<void> {
    ...
    const isVisible = await artistCombobox
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    if (!isVisible) {
  ...
  async fillReleaseDate(date: string): Promise<void> {
    await this.releaseDateInput.waitFor({
      state: "visible",
      timeout: this.timeouts.medium,
    });
