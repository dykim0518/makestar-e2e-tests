...
export interface PurchaseAppliedFilter {
  label: string;
  value: string;
}

export interface PurchaseSearchSeed {
  keyword: string;
  filters: PurchaseAppliedFilter[];
}

export interface PurchaseResultMetrics {
  rowCount: number;
...
interface PurchaseFilterControl {
  locator: Locator;
  label: string;
  kind: 'select' | 'custom';
}
...
export class PurchaseListPage extends AdminBasePage {
  ...
  constructor(page: Page) {
    ...
    this.resultSummary = page.getByText(/전체\s*[\d,]+\s*건|발주 요청관리|발주관리|입고내역/i).first();
  }
  ...
  async waitForTableOrNoResult(timeout: number = this.timeouts.navigation): Promise<void> {
    ...
    await this.page.waitForFunction(
      () => {
        ...
        return hasNoResult || hasSummary || meaningfulTableRowCount > 0 || checkboxCount > 1;
      },
      undefined,
      { timeout }
    );
  ...
  async assertTabsVisible(): Promise<void> {
    await expect(await this.resolveTab('request')).toBeVisible({ timeout: this.timeouts.long });
    await expect(await this.resolveTab('manage')).toBeVisible({ timeout: this.timeouts.long });
    await expect(await this.resolveTab('inbound')).toBeVisible({ timeout: this.timeouts.long });
  }
  ...
  async getCurrentPageNumber(defaultPage: number = 1): Promise<number> {
    ...
    const navPage = await this.page
      .evaluate(() => {
        ...
        if (ariaCurrent) {
          const parsed = parseNumber(normalize(ariaCurrent.innerText || ariaCurrent.textContent || ''));
          if (parsed !== null) {
            return parsed;
          }
        }
        ...
        if (active) {
          const parsed = parseNumber(normalize(active.innerText || active.textContent || ''));
          if (parsed !== null) {
            return parsed;
          }
        }
  ...
  async getCurrentAppliedFilters(maxControls: number = 8): Promise<PurchaseAppliedFilter[]> {
  ...
  async buildSearchSeedFromCurrentRows(maxFilters: number = 2): Promise<PurchaseSearchSeed> {
    ...
    const filters = await this.applyDynamicFiltersFromRowText(firstRow, maxFilters);
  ...
  async applySearchSeed(seed: PurchaseSearchSeed): Promise<void> {
    ...
    for (const filter of seed.filters) {
      ...
      const matchedControl =
        controls.find((control) => this.normalizeLoose(control.label) === this.normalizeLoose(filter.label)) ??
        controls.find((control) => this.normalizeLoose(control.label).includes(this.normalizeLoose(filter.label)));
      if (!matchedControl) {
        continue;
      }
  ...
  async assertRowsMatchSearchSeed(seed: PurchaseSearchSeed, sampleLimit: number = 10): Promise<void> {
    ...
    for (const filter of seed.filters) {
      ...
      const matchedRowCount = rowTexts.filter((rowText) => this.rowContainsToken(rowText, filter.value)).length;
      expect(
        matchedRowCount,
  ...
  private async resolveKeywordInput(): Promise<Locator | null> {
    return await this.findFirstVisible([
      this.page.getByRole('textbox', { name: /검색어|키워드|발주번호|입고번호|상품명|코드/i }).first(),
      this.page.getByPlaceholder(/검색어|키워드|발주번호|입고번호|상품명|코드|번호/i).first(),
  ...
  private async getFirstRowText(): Promise<string> {
    ...
    expect(rowTexts.length, '검색 가능한 목록 데이터가 없습니다.').toBeGreaterThan(0);
    return rowTexts[0];
  }
  ...
  private async getRowTexts(limit: number = 10): Promise<string[]> {
    ...
    const normalizedCheckboxTexts = checkboxBased.map((text) => this.normalize(text)).filter((text) => text.length > 0 && !text.includes('검색결과가 없습니다'));
    ...
    const tableRowTexts = await this.page
      ...
      .catch(() => [] as string[]);
    const normalizedTableTexts = tableRowTexts.map((text) => this.normalize(text)).filter((text) => text.length > 0 && !text.includes('검색결과가 없습니다'));
    ...
    return roleRowTexts.map((text) => this.normalize(text)).filter((text) => text.length > 0 && !text.includes('검색결과가 없습니다'));
  }

  private extractKeywordFromRowText(rowText: string): string {
    const normalized = this.normalize(rowText);
    const alphaNumericTokens = normalized.match(/[A-Za-z0-9][A-Za-z0-9/_\-]{3,}/g) ?? [];
    ...
    const preferredKoreanToken = koreanTokens.find((token) => token.length >= 2);
  ...
  private async applyDynamicFiltersFromRowText(rowText: string, maxFilters: number): Promise<PurchaseAppliedFilter[]> {
    const controls = await this.collectSearchFilterControls();
    const applied: PurchaseAppliedFilter[] = [];
    ...
    for (const control of controls) {
      ...
      const matchingOption = options.find((option) => this.rowContainsToken(rowText, option));
      const targetOption = matchingOption ?? options[0];
      if (!this.isMeaningfulValue(targetOption)) {
        continue;
      }

      const selected = await this.selectFilterControlOption(control, targetOption);
      if (!selected) {
        continue;
      }

      const currentValue = await this.getFilterControlCurrentValue(control).catch(() => '');
      const resolvedValue = this.isMeaningfulValue(currentValue) ? currentValue : targetOption;
  ...
  private async collectSearchFilterControls(maxControls: number = 8): Promise<PurchaseFilterControl[]> {
    const controls: PurchaseFilterControl[] = [];
    const seenLabels = new Set<string>();

    const addControl = async (locator: Locator, kind: PurchaseFilterControl['kind'], fallbackLabel: string): Promise<void> => {
      if (controls.length >= maxControls) {
        return;
      }

      const visible = await locator.isVisible({ timeout: this.timeouts.short }).catch(() => false);
      if (!visible) {
        return;
      }
      ...
      if (!this.isMeaningfulValue(label) || this.shouldIgnoreFilterLabel(label)) {
        return;
      }
    ...
    const customLocator = this.page.locator('main [role="combobox"]:visible, main .multiselect:visible');
  ...
  private async getFilterControlOptions(control: PurchaseFilterControl): Promise<string[]> {
    if (control.kind === 'select') {
      const options = await control.locator
        .evaluate((el) => {
          const select = el as HTMLSelectElement;
          return Array.from(select.options).map((option) => (option.textContent || '').replace(/\s+/g, ' ').trim());
        })
        .catch(() => [] as string[]);
  ...
  private async selectFilterControlOption(control: PurchaseFilterControl, optionText: string): Promise<boolean> {
    if (!this.isMeaningfulValue(optionText)) {
      return false;
    }

    if (control.kind === 'select') {
      const selectedByLabel = await control.locator.selectOption({ label: optionText }).catch(() => [] as string[]);
        ...
        const fallbackValue = await control.locator
          .evaluate((el, targetOption) => {
            ...
            const target = Array.from(select.options).find((option) => {
              ...
              return text === targetOption || text.includes(targetOption as string);
            });
            return target?.value || '';
          }, optionText)
          .catch(() => '');
    ...
    let clicked = false;
    if (await exactOption.isVisible({ timeout: this.timeouts.short }).catch(() => false)) {
    ...
    if (!clicked) {
      const partialOption = this.page
        ...
        .first();
      if (await partialOption.isVisible({ timeout: this.timeouts.short }).catch(() => false)) {
  ...
  private async getFilterControlCurrentValue(control: PurchaseFilterControl): Promise<string> {
    if (control.kind === 'select') {
  ...
  private async inferFilterLabel(locator: Locator, fallback: string): Promise<string> {
  ...
  private async resolveTab(tab: PurchaseTabKey): Promise<Locator> {
    ...
    const resolved = await this.findFirstVisible([
      ...
      headingArea.locator('div, button, a, span, p').filter({ hasText: exactRegex }).first(),
