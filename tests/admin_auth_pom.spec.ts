 ...
 * Admin 인증 Setup 테스트
 *
 * 어드민 페이지 접근 권한을 확인하고, 현재 auth 상태가 실제로 동작하는지 검증합니다.
 * 이 테스트가 Setup Project로 먼저 실행되며, 성공해야만 다른 Admin 테스트가 실행됩니다.
 *
 * 흐름:
 * 1. 토큰 유효성 확인
 * 2. auth.json 쿠키 주입
 * 3. protected route 접근 및 인증 검증
 * 4. 인증 성공 → 다른 Admin 테스트 실행 가능
 *
...
test.beforeAll(async () => {
  ...
  if (tokenValid) {
    ...
    console.warn("\n⚠️ auth.json의 refresh_token이 없거나 만료되었습니다.");
    console.warn("   로컬에서 npm run auth:refresh 를 실행해 갱신하세요.\n");
  }
});
...
test.describe.serial("인증 검증", () => {
  test("AUTH-VERIFY-01: 어드민 페이지 접근 인증 확인", async ({ page }) => {
    // 이 테스트가 실패하면 이후 Admin 테스트도 동일한 인증 실패 상태를 공유함
    ...
    if (!authResult.success) {
      ...
      console.error(`\n❌ 최종 인증 실패: ${failReason}`);
      console.log("\n🔧 수동 해결 방법:");
      console.log("   로컬: npm run auth:refresh");
      if (process.env.CI) {
        console.log(
          "   CI: 최신 auth.json을 다시 업로드해 AUTH_JSON / STG_AUTH_JSON secret을 갱신",
        );
      }
      console.warn("\n⚠️ 이후 모든 테스트는 실패 상태를 공유합니다.\n");
