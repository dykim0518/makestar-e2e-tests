...
async function checkExistingSession(page: Page): Promise<boolean> {
  ...
  try {
    ...
    await page.goto(`${ALBUMBUDDY_BASE_URL}/dashboard/purchasing`, {
...
test("AlbumBuddy 로그인 세션 저장 (수동 로그인)", async ({
  ...
  await page.goto(`${ALBUMBUDDY_BASE_URL}/shop`, {
    waitUntil: "domcontentloaded",
  });
...
test("기존 AlbumBuddy 세션 유효성 확인", async ({ page }) => {
  ...
  const currentUrl = page.url();
  const isLoggedIn =
    currentUrl.includes("purchasing") && !currentUrl.includes("login");
