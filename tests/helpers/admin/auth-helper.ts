 ...
 * 인증된 URL 생성
 */
export function getAuthenticatedUrl(targetPath: string, includeToken: boolean = true): string {
  if (!includeToken) return BASE_URL + targetPath;
  try {
    if (fs.existsSync(AUTH_FILE)) {
      const authData: AuthData = JSON.parse(fs.readFileSync(AUTH_FILE, 'utf-8'));
      const rtCookie = authData.cookies?.find((c) => c.name === 'refresh_token');
      if (rtCookie?.value) {
    ...
    if (authData.cookies && authData.cookies.length > 0) {
      ...
      for (const raw of authData.cookies) {
        ...
        const base = {
          ...
          secure: !!raw.secure,
          sameSite: (['Lax', 'None', 'Strict'].includes(raw.sameSite ?? '') ? raw.sameSite : 'Lax') as 'Lax' | 'None' | 'Strict',
          ...(typeof raw.expires === 'number' ? { expires: Math.floor(raw.expires) } : {}),
        };
      ...
      if (cookiesToSet.length > 0) {
        await context.addCookies(cookiesToSet as Parameters<typeof context.addCookies>[0]);
    ...
    if (ls) {
      ...
      if (lsAccess && lsRefresh) {
        await page.addInitScript((tokens) => {
          ...
          userInfo: (() => { try { return JSON.parse(ls['user_info'] ?? 'null'); } catch { return null; } })(),
...
export function getSystemToken(): string | null {
  ...
  try {
    ...
    const tokens: AdminTokens = JSON.parse(fs.readFileSync(ADMIN_TOKENS_FILE, 'utf-8'));
...
export async function setupApiInterceptor(page: Page): Promise<boolean> {
  ...
  if (!token) {
    console.warn('⚠️ 시스템 토큰을 찾을 수 없어 API 인터셉터를 설정하지 않습니다.');
    return false;
  }

  // Admin API 엔드포인트 패턴
  const adminApiPattern = /^https:\/\/(stage-new-admin|stage-api|stage-auth)\.makeuni2026\.com\/.*/;
