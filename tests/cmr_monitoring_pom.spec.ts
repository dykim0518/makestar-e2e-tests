 ...
 *
 * E. 마이페이지/회원 기능 (CMR-AUTH-01~13, CMR-AUTH-04-1~04-2)
 *    - 마이페이지, 주문내역, 배송지, 비밀번호, 응모정보, 이메일 로그인, 수정/등록 폼
 *
...
test.describe("기본 페이지", () => {
  ...
  // ------------------------------------------------------------------------
  test(
    "CMR-HOME-01: 메인 페이지 접속 및 초기 모달 처리",
    { tag: "@feature:cmr.home" },
    async () => {
  ...
  // ------------------------------------------------------------------------
  test(
    "CMR-PAGE-01: Event 페이지 이동 및 요소 검증",
    { tag: "@feature:cmr.event.list" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-PAGE-02: Event 종료된 이벤트 탭 이동 및 검증",
    { tag: "@feature:cmr.event.list" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-PAGE-03: Event 진행중인 이벤트 탭 및 첫 번째 상품 클릭",
    { tag: ["@feature:cmr.event.list", "@feature:cmr.product.detail"] },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  // ------------------------------------------------------------------------
  test(
    "CMR-PAGE-04: Product 페이지 주요 요소 검증 및 옵션 선택",
    { tag: "@feature:cmr.product.detail" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-PAGE-05: Product 구매하기 클릭 및 결과 검증",
    { tag: "@feature:cmr.product.detail" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
...
test.describe("GNB 네비게이션", () => {
  ...
  test(
    "CMR-NAV-01: Shop 페이지 이동 및 요소 검증",
    { tag: "@feature:cmr.shop.list" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-NAV-02: Funding 페이지 이동 및 요소 검증",
    { tag: "@feature:cmr.funding" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-NAV-04: Home 버튼으로 메인 페이지 복귀",
    { tag: "@feature:cmr.home" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
...
test.describe("검색 기능", () => {
  ...
  test(
    "CMR-SEARCH-01: 검색 UI 열기 및 추천 검색어 표시 확인",
    { tag: "@feature:cmr.search.overlay" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
      await makestar.handleModal();
  ...
  test(
    "CMR-SEARCH-02: 검색어 입력 및 검색 결과 확인",
    { tag: "@feature:cmr.search.results" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
      await makestar.handleModal();
  ...
  test(
    "CMR-SEARCH-03: 검색 결과 페이지 UI 및 결과 표시 확인",
    { tag: "@feature:cmr.search.results" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-SEARCH-04: 검색 결과 필터링 (카테고리/탭) 확인",
    { tag: "@feature:cmr.search.filters" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-SEARCH-05: 검색 후 추천 검색어 재표시 확인",
    { tag: "@feature:cmr.search.overlay" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
...
test.describe.serial("네비게이션 검증", () => {
  ...
  test(
    "CMR-NAV-05: 프로필 버튼 → 마이페이지 네비게이션",
    { tag: "@feature:cmr.mypage.main" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-NAV-06: GNB Shop 버튼 클릭 → 상품 목록 페이지",
    { tag: "@feature:cmr.shop.list" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-NAV-07: GNB Event 버튼 클릭 → 이벤트 목록 페이지",
    { tag: "@feature:cmr.event.list" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-NAV-08: GNB Funding 버튼 클릭 → 펀딩 목록 페이지",
    { tag: "@feature:cmr.funding" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
...
test.describe("마이페이지/회원 기능", () => {
  ...
  test(
    "CMR-AUTH-01: 마이페이지 접속 및 프로필 정보 확인",
    { tag: "@feature:cmr.mypage.main" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-03: 주문내역 페이지 이동 및 확인",
    { tag: "@feature:cmr.mypage.order-history" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-04: 배송지 관리 페이지 이동 및 확인",
    { tag: "@feature:cmr.mypage.address" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-04-1: 팔로우 관리 페이지 이동 및 확인",
    { tag: "@feature:cmr.mypage.follow" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-04-2: 알림 설정 페이지 이동 및 확인",
    { tag: "@feature:cmr.mypage.notification" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-05: 비밀번호 변경 페이지 접근 및 요소 검증",
    { tag: "@feature:cmr.mypage.password" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-06: 이벤트 응모정보 관리 페이지 검증",
    { tag: "@feature:cmr.mypage.event-submissions" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-07: 이메일 로그인 — 이메일 입력 시 다음 버튼 활성화 검증",
    { tag: "@feature:cmr.mypage.main" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-08: auth.json 세션 기반 로그인 상태 검증",
    { tag: "@feature:cmr.mypage.main" },
    async ({ page }) => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-AUTH-09: 내 정보 수정 페이지 접근 및 기본 프로필 폼 검증",
    { tag: "@feature:cmr.mypage.update" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);

      await makestar.gotoProfileUpdate();
      await makestar
        .waitForContentStable("body", { stableTime: 500, timeout: 3000 })
        .catch(() => {});

      expect(
        makestar.currentUrl.includes("/my-page/update"),
        `내 정보 수정 URL이어야 합니다 (현재: ${makestar.currentUrl})`,
      ).toBe(true);

      const hasContent = await makestar.hasProfileUpdateContent();
      expect(hasContent, "프로필 수정 콘텐츠가 표시되어야 합니다").toBe(true);

      const saveDisabled = await makestar.isSaveButtonDisabled();
      expect(
        saveDisabled,
        "초기 진입 시 Save 버튼은 변경 전까지 비활성화 상태여야 합니다",
      ).toBe(true);
    },
  );

  test(
    "CMR-AUTH-10: 배송지 등록 페이지 접근 및 입력 폼 검증",
    { tag: "@feature:cmr.mypage.address.create" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);

      await makestar.gotoAddressCreate();
      await makestar
        .waitForContentStable("body", { stableTime: 500, timeout: 3000 })
        .catch(() => {});

      expect(
        makestar.currentUrl.includes("/my-page/address/post"),
        `배송지 등록 URL이어야 합니다 (현재: ${makestar.currentUrl})`,
      ).toBe(true);

      const hasContent = await makestar.hasAddressFormContent();
      expect(hasContent, "배송지 등록 폼 콘텐츠가 표시되어야 합니다").toBe(
        true,
      );
      ...
      expect(
        saveDisabled,
        "빈 배송지 등록 폼의 Save 버튼은 비활성화 상태여야 합니다",
      ).toBe(true);
    },
  );

  test(
    "CMR-AUTH-11: 배송지 수정 페이지 접근 및 기존 값 로드 검증",
    { tag: "@feature:cmr.mypage.address.update" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);

      const href = await makestar.gotoFirstAddressUpdate();
      await makestar
        .waitForContentStable("body", { stableTime: 500, timeout: 3000 })
        .catch(() => {});

      expect(
        /\/my-page\/address\/update\//.test(makestar.currentUrl),
        `배송지 수정 URL이어야 합니다 (현재: ${makestar.currentUrl})`,
      ).toBe(true);
      expect(href.includes("/my-page/address/update/")).toBe(true);

      const hasContent = await makestar.hasAddressFormContent();
      expect(hasContent, "배송지 수정 폼 콘텐츠가 표시되어야 합니다").toBe(
        true,
      );
  ...
  test(
    "CMR-AUTH-12: 이벤트 응모 등록 페이지 접근 및 입력 폼 검증",
    { tag: "@feature:cmr.mypage.event-submissions.create" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);

      await makestar.gotoEventSubmissionCreate();
      await makestar
        .waitForContentStable("body", { stableTime: 500, timeout: 3000 })
        .catch(() => {});

      expect(
        makestar.currentUrl.includes("/my-page/event-submissions/post"),
        `이벤트 응모 등록 URL이어야 합니다 (현재: ${makestar.currentUrl})`,
      ).toBe(true);

      const hasContent = await makestar.hasEventEntryFormContent();
      expect(hasContent, "이벤트 응모 등록 폼 콘텐츠가 표시되어야 합니다").toBe(
        true,
      );
      ...
      expect(
        saveDisabled,
        "빈 이벤트 응모 등록 폼의 Save 버튼은 비활성화 상태여야 합니다",
      ).toBe(true);
    },
  );

  test(
    "CMR-AUTH-13: 이벤트 응모 수정 페이지 접근 및 기존 정보 로드 검증",
    { tag: "@feature:cmr.mypage.event-submissions.update" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);

      const href = await makestar.gotoFirstEventSubmissionUpdate();
      await makestar
        .waitForContentStable("body", { stableTime: 500, timeout: 3000 })
        .catch(() => {});

      expect(
        /\/my-page\/event-submissions\/update\//.test(makestar.currentUrl),
        `이벤트 응모 수정 URL이어야 합니다 (현재: ${makestar.currentUrl})`,
      ).toBe(true);
      expect(href.includes("/my-page/event-submissions/update/")).toBe(true);

      const hasContent = await makestar.hasEventEntryFormContent();
      expect(hasContent, "이벤트 응모 수정 폼 콘텐츠가 표시되어야 합니다").toBe(
        true,
      );
      ...
      expect(
        hasSaveButton,
        "이벤트 응모 수정 페이지에 Save 버튼이 있어야 합니다",
      ).toBe(true);
    },
  );
});
...
test.describe("상품/장바구니 기능", () => {
  ...
  test(
    "CMR-ACTION-01: 상품 옵션 변경에 따른 가격 변동 확인",
    { tag: "@feature:cmr.product.detail" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-ACTION-03: 장바구니 담기 및 수량 변경 검증",
    { tag: "@feature:cmr.cart" },
    async () => {
  ...
  test(
    "CMR-ACTION-05: 장바구니 수량 증가 시 가격 반영 검증",
    { tag: "@feature:cmr.cart" },
    async () => {
  ...
  test(
    "CMR-ACTION-04: 비회원 상태에서 홈/이벤트 페이지 정상 접근 확인",
    { tag: ["@feature:cmr.home", "@feature:cmr.event.list"] },
    async ({ browser }) => {
      test.setTimeout(TEST_TIMEOUT);
...
test.describe("아티스트/콘텐츠", () => {
  ...
  test(
    "CMR-DATA-01: 아티스트 프로필 페이지 접근 및 정보 표시 확인",
    { tag: "@feature:cmr.artist.detail" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
  ...
  test(
    "CMR-DATA-02: 아티스트별 상품 목록 필터링 확인",
    { tag: "@feature:cmr.product.detail" },
    async () => {
      test.setTimeout(TEST_TIMEOUT);
